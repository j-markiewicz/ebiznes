...
suspend fun main() {
	...
	val proxy_url = dotenv["PROXY_URL"] ?: "http://localhost:8000"
	...
	println("Bot starting with api base $api_base and LLM proxy at $proxy_url as Bot ${client_secret.subSequence(0, client_secret.indexOfLast {it == '.'} + 4)}...")
				...
				"MESSAGE_CREATE" -> {
					...
					if (is_bot) {
						...
						if (content.contains("llm")) {
							val res: HttpResponse = client.post(proxy_url) {
								setBody(content)
								timeout {
							...
							if (res.status.value in 200..299) {
								val response: String = res.body()
								println(response)
								send_message(client, headers, api_base, channel_id, response)
							}
						} else {
							var sent = false
